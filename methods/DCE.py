...
def DCE(output, label, expert):
    ...
    expert_score = output[:,-1]
    index = torch.arange(0,class_score.size(-1),1).unsqueeze(0).to(device=DEVICE) # B, K
    index = index.expand(class_score.size(0),-1) # B, K
    _, max_index = torch.max(class_score,dim=-1) # B, 1
    
    neg_score = class_score[index!=max_index.unsqueeze(-1)].reshape(class_score.size(0),-1)
    
    loss_expert = torch.cat([expert_score.unsqueeze(-1),neg_score],dim=-1)
    loss_expert = torch.log(torch.softmax(loss_expert,dim=-1)[:,0]+1e-7)
    
    loss_classifier0 = torch.log(torch.softmax(output,dim=-1)+1e-7)[:,0:-1][index==label.unsqueeze(-1)]
    loss_classifier1 = torch.log(torch.softmax(class_score,dim=-1)+1e-7)[index==label.unsqueeze(-1)]
    loss =  - ((loss_classifier1 + loss_expert) * expert_correctness).sum()/class_score.size(0) \
    - (loss_classifier0 * (1-expert_correctness)).sum()/class_score.size(0)
    return loss
...
def main(cfg, args):
    ...
    logdir = os.path.join(cfg.experiment.logdir, cfg.dataset.name, 'coverage_'+str(args.coverage)+'_DCE')
    modeldir = os.path.join(cfg.experiment.modeldir, cfg.dataset.name, 'coverage_'+str(args.coverage)+'_DCE')
    ...
    try:
        ...
        for epoch_id in tqdm(
            ...
            for batch_idx, (inputs, labels, expert) in enumerate(train_loader):
                ...
                loss = DCE(output=output, label=labels, expert=expert)
...
if __name__ == '__main__':
    ...
    cfg = OmegaConf.load('../config/dce_chestxray_conf.yaml')
    # cfg = OmegaConf.load('../config/dce_chaoyang_conf.yaml')
