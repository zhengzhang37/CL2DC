...
def Asym_SM_trans(scorer):
    ...
    classifier_input = classifier_input.to(device=DEVICE)
    output1 = torch.softmax(classifier_input, dim=-1).cuda()

    sm = torch.softmax(scorer, dim=-1).to(device=DEVICE)
...
def Asym_CE(output, label, expert):
    ...
    output_probit = output_probit.to(device=DEVICE)
    sm = output_probit[:, 0:-1]
    bsm = output_probit[:, -1]
...
def main(cfg, args):
    ...
    scheduler = CosineAnnealingLR(optimiser, len(train_loader) * 200, eta_min=0)
...
if __name__ == '__main__':
    ...
    # cfgs = ['../config/asymce_chaoyang_conf.yaml', '../config/asymce_chestxray_conf.yaml']
    cfgs = ['../config/asymce_cifair_conf.yaml']
