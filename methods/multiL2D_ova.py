...
def loss_fn(outputs, labels, collection_Ms, n_classes, n_experts):
    '''
    ...
    l2 = torch.sum(
        LogisticLoss(outputs[:, :n_classes], -1), dim=1
    ) - LogisticLoss(outputs[range(batch_size), labels], -1)

    l3 = 0
    for j in range(n_experts):
        l3 += LogisticLoss(outputs[range(batch_size), n_classes + j], -1)

    l4 = 0
    for j in range(n_experts):
        l4 += collection_Ms[j] * (LogisticLoss(outputs[range(
            batch_size), n_classes + j], 1) 
            - LogisticLoss(outputs[range(batch_size), n_classes + j], -1))
    ...
    try:
        ...
        for epoch_id in tqdm(
            ...
            for batch_idx, (inputs, labels, experts) in enumerate(train_loader):
                ...
                collection_Ms = []
                for i in range(cfg.dataset.num_users):
                    exp_prediction = experts[:, i]
                    m = torch.zeros_like(exp_prediction)
                    m[exp_prediction == labels] = 1
                    m = m.to(device=DEVICE)
                    collection_Ms.append(m)
                loss = loss_fn(output, labels, collection_Ms, cfg.dataset.num_classes, cfg.dataset.num_users)
...
if __name__ == '__main__':
    ...
    cfgs = [
        # '../config/multil2d_chaoyang2u_conf.yaml', 
        # '../config/multil2d_chaoyang3u_conf.yaml', 
        # '../config/multil2d_micebone_conf.yaml', 
        # '../config/multil2d_chestxray_conf.yaml', 
        # '../config/multil2d_ham10000_conf.yaml', 
        '../config/multil2d_cifair_conf.yaml',
        '../config/multil2d_galaxyzoo_conf.yaml'
        ]
